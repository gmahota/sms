Meteor.methods({
    'compareClassGenderResult': function(yearId, termName, form, stream, gender) {
        if (Meteor.isServer) {
            var webshot = Meteor.npmRequire('webshot');
            var fs      = Npm.require('fs');
            var Future = Npm.require('fibers/future');
            var fut = new Future();
            var fileName = "compare-class-gender-results.pdf";
            ...
            SSR.compileTemplate('template', Assets.getText('compare-class-gender-results.html'));
            //========================================
            ...
            var studentIdArray = Students.find({class: classId, gender: gender, active: true}).map(function(student){
                return student._id;
            });

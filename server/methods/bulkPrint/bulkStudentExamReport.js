Meteor.methods({
    'bulkStudentExamReport': function(examId, classId) {
        if (Meteor.isServer) {
          // SETUP
          ...
          var fileName = "bulk-student-report.pdf";

          // GENERATE HTML STRING
          var css = Assets.getText('merged-stylesheets.css');

          SSR.compileTemplate('layout', Assets.getText('bulkLayoutPortrait.html'));
          ...
          SSR.compileTemplate('bulk_student_report', Assets.getText('bulk-student-report.html'));

          // PREPARE DATA
            var studentIdArr = Students.find({class: classId, active: true}).map(function(student){
                return student._id;
            });
            var singleData = [];

            var allData = Results.find({student: {$in: studentIdArr}, exam: examId}).map(function(result){
                ...
                var subjectCount = result.subjects.length;

                var subjectsArray = [];
                var subjectData = result.subjects.map(function(subject){
